...
namespace ImageSharp.Benchmarks.MAUI.Benchmarks;
...
public class LoadResizeSave
{
    ...
    //private readonly ImageCodecInfo systemDrawingJpegCodec = ImageCodecInfo.GetImageEncoders().First(codec => codec.FormatID == ImageFormat.Jpeg.Guid);

    public string[] Images { get; private set; }

    public double TotalProcessedMegapixels { get; private set; }

    public SystemDrawingSize LastProcessedImageSize { get; private set; }
    ...
    public LoadResizeSave()
    {
        ...
        ImageCount = 1; // Environment.ProcessorCount;
        Filter = Filter;
        ...
        {
            if (res.Contains(".Images.Input.MemoryStress."))
            {
                inputImages.Add(res);
            }
        }
        ...
        string imageDirectory = Path.Combine("Images", "Input", "MemoryStress");
        if (!inputImages.Any())
        {
        ...
        this.Images = inputImages.Take(this.ImageCount).ToArray();



        // Create the output directory next to the images directory
        this.outputDirectory = CreateOutputDirectory("MemoryStress");

        // Copy images out of embedded resources.
        var inputImagesPath = CreateOutputDirectory("MemoryStress", "InputImages");
        for (int i = 0; i < Images.Length; ++i)
        {
            var filename = Path.GetFileName(Images[i]);
            var outputFilename = Path.Combine(inputImagesPath, filename);
            using (var inputStream = assembly.GetManifestResourceStream(Images[i]))
            {
    ...
    private void ForEachImage(Action<string> action, int maxDegreeOfParallelism)
    {
        ...
        this.ForEachImageParallel(action);
    }
    ...
    {
        this.LastProcessedImageSize = new SystemDrawingSize(width, height);
    ...
    //public void SystemDrawing(int maxDegreeOfParallelism) => this.ForEachImage(this.SystemDrawingResize, maxDegreeOfParallelism);
    ...
    //[ArgumentsSource(nameof(ParallelismValues))]
    public void ImageSharp() => this.ForEachImage(this.ImageSharpResize, MaxDegreeOfParallelism);
    ...
    //[Benchmark]
    //[ArgumentsSource(nameof(ParallelismValues))]
    //public void Magick(int maxDegreeOfParallelism) => this.ForEachImage(this.MagickResize, maxDegreeOfParallelism);
    ...
    //[Benchmark]
    //[ArgumentsSource(nameof(ParallelismValues))]
    //public void MagicScaler(int maxDegreeOfParallelism) => this.ForEachImage(this.MagicScalerResize, maxDegreeOfParallelism);
    ...
    [Benchmark]
    //[ArgumentsSource(nameof(ParallelismValues))]
    public void SkiaBitmap() => this.ForEachImage(this.SkiaBitmapResize, MaxDegreeOfParallelism);
    ...
    [Benchmark]
    //[ArgumentsSource(nameof(ParallelismValues))]
    public void SkiaBitmapDecodeToTargetSize() => this.ForEachImage(this.SkiaBitmapDecodeToTargetSize, MaxDegreeOfParallelism);
    ...
    //[Benchmark]
    //[ArgumentsSource(nameof(ParallelismValues))]
    //public void NetVips(int maxDegreeOfParallelism) => this.ForEachImage(this.NetVipsResize, maxDegreeOfParallelism);
